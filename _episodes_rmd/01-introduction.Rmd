...
We would like to introduce the data that we will be using during the course. The Critical Care Health Informatics Collaborative (CC-HIC) is a UK research body that has aggregated data from thousands of critical care patients. We will be using a synthetic dataset based on this cohort.

The data has been pre-prepared for us today, and will be presented as a "spread sheets" or what is called a `data.frame` in `R` parlance. This is the most common format for presenting data that can be described in rows and columns (so called "rectangular data").
